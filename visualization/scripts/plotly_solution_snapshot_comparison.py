...
def make_plotly_figure(
    ...
    figure = make_subplots(
        ...
        subplot_titles=[
            f"Initial deterministic  t={format_time_value(times[0])}",
            f"Initial stochastic  t={format_time_value(times[0])}",
            f"Final deterministic  t={format_time_value(times[-1])}",
            f"Final stochastic  t={format_time_value(times[-1])}",
        ],
    )
    ...
    vmin, vmax, colorscale_name = choose_color_scale(display_fields)
    colorscale_name = colorscale or colorscale_name
    zmin, zmax = shared_value_range(display_fields)
    ...
    for kind, time_value, row, col in panel_order:
        ...
        trace = go.Surface(
            ...
            colorbar=dict(title="Solution",
                          lenmode="fraction", len=0.62, y=0.44),
            contours_z=dict(
                show=True,
                usecolormap=True,
    ...
    camera_golden = dict(eye=dict(x=1.75, y=-1.95, z=0.78))
    camera_iso = dict(eye=dict(x=1.55, y=-1.55, z=1.05))
    camera_top = dict(eye=dict(x=0.0, y=0.0, z=3.0))
    camera_side = dict(eye=dict(x=2.8, y=0.0, z=0.5))
    ...
    scene_template = dict(
        ...
        zaxis=dict(showgrid=True, zeroline=False, range=[zmin, zmax]),
    )

    for scene_id in range(1, 5):
        figure.update_layout({f"scene{scene_id}": scene_template})
    ...
    figure.update_layout(
        title="Initial and final deterministic/stochastic solution snapshots",
        width=width,
        height=height,
    ...
    if tracker_index is not None:
        ...
        figure.add_annotation(
            ...
            xanchor="right",
            text=(
                "Tracker: max final |noise effect| at "
                f"(x={tracker_x:.4g}, y={tracker_y:.4g}), "
                f"delta={final_difference:.4g}"
            ),
            showarrow=False,
            font=dict(size=12),
        )

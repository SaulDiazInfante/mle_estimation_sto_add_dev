...
module model_types_mod
    ...
    !> Spatial and physical configuration for the rectangular computational grid.
    type, public :: spatial_grid_t
        integer :: nx = 10                !!< Number of grid cells or modes in x.
        integer :: ny = 10                !!< Number of grid cells or modes in y.
        ...
        real(dp) :: gamma = 2.0_dp        !!< Fractional exponent used in diffusion scaling.
    ...
    type, public :: spectral_operator_set_t
        integer :: state_dimension = 0                 !!< Number of active non-zero modes.
        integer, allocatable :: mode_pairs(:, :)       !!< Modal indices sorted by eigenvalue.
        real(dp), allocatable :: initial_state(:)      !!< Projected initial modal state.
        real(dp), allocatable :: eigenvalues(:)        !!< Eigenvalues of the spectral basis.
        real(dp), allocatable :: diffusion_diagonal(:) !!< Diagonal diffusion weights.
        real(dp), allocatable :: interaction_matrix(:, :) !!< Linear interaction operator.
    ...
    type, public :: parameter_estimates_t
        ...
        real(dp) :: beta_hat = 0.0_dp        !!< Estimated dissipative coefficient.
        real(dp) :: theta_hat = 0.0_dp       !!< Estimated coupling coefficient.
        real(dp) :: setup_time = 0.0_dp      !!< CPU time spent assembling and simulating.
        real(dp) :: estimation_time = 0.0_dp !!< CPU time spent in the estimation stage.
